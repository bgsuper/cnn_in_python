...
def affine_backward(dout, cache):
    ...
    ###########################################################################
    dx_ = np.dot(dout, np.transpose(w))
    dx = np.reshape(dx_, x.shape)
    x_ = np.reshape(x, [N, -1])
    
    dw = np.dot(np.transpose(x_), dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_normal = (x - sample_mean)/np.sqrt(sample_var+eps)
        out = gamma*sample_normal + beta
        cache = {'x': x, 
                 'gamma':gamma, 
                 'beta':beta, 
        ...
        #######################################################################
        sample_normal = (x -running_mean)/np.sqrt(running_var + eps)
        out = gamma*sample_normal + beta
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, gamma,beta, sample_mean, sample_var, sample_normal, eps= \
        cache['x'], cache['gamma'],cache['beta'], cache['sample_mean'], \
        cache['sample_var'], cache['sample_normal'], cache['eps']
    N = x.shape[0]
    dgamma = np.sum(dout*sample_normal, axis=0)
    dbeta = np.sum(dout, axis=0)
    dout_=dout*gamma
    dx = (N*dout_ - np.sum(dout_, axis=0) - \
          sample_normal*(np.sum(dout_*sample_normal, axis=0)))/(N*np.sqrt(sample_var + eps))
...
def dropout_forward(x, dropout_param):
    ...
    mask = np.random.binomial(1, p, size=N)/p
...
def max_pool_forward_naive(x, pool_param):
    ...
    # x_col has shape (C*HH*WW, N*H_new*W_new)
    x_col = im2col_indices(x_, pool_height, pool_width, padding=0, stride=stride)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    running_mean = bn_param.get('running_mean', np.zeros([C, 1], dtype=x.dtype))
    running_var = bn_param.get('running_var', np.zeros([C, 1], dtype=x.dtype))
    
    x_ = x.transpose(1,2,3,0)
    x_ = x_.reshape(C, -1)
    if mode == 'train':
        #######################################################################
        # TODO: Implement the training-time forward pass for batch norm.      #
        ...
        sample_mean, sample_var = None, None
        sample_mean = np.mean(x_, axis=1).reshape([C, 1])
        sample_var = np.var(x_, axis=1).reshape([C, 1])
        running_mean = momentum*running_mean + (1-momentum)*sample_mean
        running_var = momentum*running_var + (1-momentum)*sample_var
        #print(sample_mean.shape)
        sample_normal = (x_ - sample_mean)/np.sqrt(sample_var+eps)
        gamma = gamma.reshape([C, 1])
        beta = beta.reshape([C, 1])
        out_ = gamma*sample_normal + beta
        cache = {'x': x, 
                 'gamma':gamma, 
                 'beta':beta, 
                 'sample_mean':sample_mean,
        ...
        out = out_.reshape(C, H, W, N).transpose(3,0,1,2)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
        ...
        sample_normal = (x_ -running_mean)/np.sqrt(running_var + eps)
        gamma = gamma.reshape([C, 1])
        beta = beta.reshape([C, 1])
        out_ = gamma*sample_normal + beta
        out = out_.reshape(C, H, W, N).transpose(3,0,1,2)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout*sample_normal, axis=1)
    dbeta = np.sum(dout, axis=1)
    dout_=dout*gamma.reshape(C, 1)
    dx_ = (N*H*W*dout_ - np.sum(dout_, axis=1).reshape(C, 1) - \
          sample_normal*(np.sum(dout_*sample_normal, axis=1).reshape(3,1)))/(N*H*W*np.sqrt(sample_var.reshape(C, 1) + eps))
    
    dx = dx_.reshape(C, H, W, N).transpose(3,0,1,2)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
